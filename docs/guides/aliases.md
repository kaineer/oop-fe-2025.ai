...
- **Важно:** REF комментарии удаляются только в копируемых файлах нового шага, в исходных файлах они сохраняются
- Выполняет проверку соответствия между метаданными и кодом исходного шага с помощью `step-refs`
  - Проверяет, что все REF метки в коде зарегистрированы в `meta.yaml` **для этого конкретного шага**
  - Проверяет, что все REF ссылки в `meta.yaml` **для этого конкретного шага** существуют в коде
  - Выдает предупреждения для несоответствий с указанием имени файла и метки
...
#### `step-refs`
Проверяет соответствие REF меток между кодом и метаданными

- При вызове без параметров проверяет ref-ы для текущего шага
- При вызове с одним параметром проверяет шаг с этим номером
- При вызове с двумя параметрами проверяет шаги, начиная с первого параметра и вплоть до последнего (включительно)
- Выдает отчет с указанием имени файла и метки для несоответствий
- Используется внутри `step-copy` для автоматической проверки
